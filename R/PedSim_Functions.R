...
#'
#' \code{create_pedFile} initializes an empty data frame with all of the
#' fields required by \code{sim_ped} to simulate a pedigree.
#'
#' @return An empty data frame with all the required fields for generating
#' a ped file.
#' @export
#'
...
#'
#' @param partner_info  Data.frame with 1 row. All ped file information for the
#'  individual requiring a new mate, must contain all fields generated by
#'   \code{\link{create_pedFile}}.
...
#' @param dad_info Data.frame with 1 row. All ped file information of offspring's father, must contain all fields generated by \code{\link{create_pedFile}}.
#' @param mom_info Data.frame with 1 row. All ped file information of offspring's mother, must contain all fields generated by \code{\link{create_pedFile}}.
...
#' appropriate.
#'
#' @param found_info Data.frame with 1 row. All ped file information for the founder, must contain all fields generated by \code{\link{create_pedFile}}.
#' @inheritParams create_offspring
#' @inheritParams sim_RVpedigree
#'
#' @return A ped file with all updated life events for the founder, and
...
#'
#' Please note distinction between \code{sim_ped} and \code{sim_RVpedigree}.  Pedigrees simulated using \code{sim_ped} do not account for study design.  To simulate a pedigree ascertained to contain multiple family members affected by a disease please use \code{\link{sim_RVpedigree}}.
#'
#' By assumption, all pedigrees are segregating a rare variant, which is rare enough to have been introduced by exactly one founder.  \code{sim_ped} starts simulating the pedigree by generating the birth year for this founder, uniformly between the years specified by \code{founder_byears}.  Next, all life events are simulated for the founder via \code{\link{get_lifeEvents}}.  Possible life events include: reproduction, disease onset, and death.  Currently, \code{sim_ped} only allows disease onset to occur once, i.e. no remission after disease onset.  Compuationally, this implies that after an individual has experienced disease onset, their waiting time to death is always simulated using the age-specific mortaility rates for the \emph{affected} population (i.e. the second column specified in \code{death_hazard}).  Life events for individuals who have inherited the rare variant are simulated such that their relative risk of developing disease is \code{RR}, according to a Cox proportional hazards model. For all individuals who have not inherited the rare variant the relative risk of disease onset is 1.  Any life events that occur after \code{stop_year} are censored.
#'
...
#' @inheritParams sim_RVpedigree
#'
#' @return The ped file of the simulated pedigree.
#' @export
#' @importFrom stats runif
#'
#' @section See Also:
...
#'
#' \code{sim_RVpedigree} simulate a pedigrees ascertained to contain multiple affected members, selects a proband, and trims the pedigree so that it contains only individuals that are recalled by the proband.
#'
...
#'
#' \code{sim_RVpedigree} begins pedigree simulation by generating the year of birth, uniformly, between the years specified in \code{founder_byears} for the founder who introduced the rare variant to the pedigree.  Next, we simulate this founder's life events using the \code{\link{get_lifeEvents}} function, and censor any events that occur after \code{stop_year}.  Possible life events include: reproduction, disease onset, and death. We continue simulating life events for any offpring, censoring events which occur after the stop year, until the simulation process terminates.
#'
#' \code{sim_RVpedigree} will only return ascertained pedigrees with at least \code{num_affected} affected individuals, that is if a simulated pedigree does not contain at least \code{num_affected} affected individuals \code{sim_RVpedigree} will discard that pedigree and simulate another until the condition is met.  We note that even for \code{num_affected} \eqn{= 2}, \code{sim_RVpedigree} can be computationally expensive.  To randomly simulate a pedigree without a desired number of affecteds use instead \code{\link{sim_ped}}.
#'
...
#' @param RR Numeric. The relative risk of disease for individuals who have inherited the rare variant.
