...
#' @param intro_RV Logical. If \code{intro_RV = TRUE}, then at least 1 founder has introduced the causal variant to the pedigree. If \code{intro_RV = FALSE} then the variant has not yet been introduced to the pedigree.
#'
...
create_mate = function(partner_info, last_id,
  ...
  new_mate_info$RR <- founder_dat[[2]]
  IRV <- founder_dat[[3]]

  mate_return <- list(new_mate_info, last_id + 1, IRV)
  return(mate_return)
}
...
simPed = function(hazard_rates, GRR,
  ...
  if (carrier_prob <= 0 | carrier_prob >= 1){
    ...
    warning('carrier_prob > 0.002: simRVped is intended for simulating the transmission of rare variants.')
  }
...
#'
#' \code{simRVped} simulates a pedigree ascertained to contain multiple affected members, selects a proband, and trims the pedigree to contain only those individuals that are recalled by the proband.
#'
...
#'
#' In the event that a trimmed pedigree fails the \code{num_affected} condition,  \code{simRVped} will discard that pedigree and simulate another until the condition is met.  For this reason, the values specified for \code{recall_probs} affect computation time.
#'
