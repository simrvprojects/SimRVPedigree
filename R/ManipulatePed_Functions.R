...
#'
#' Users who wish to assign generation number based on affection status in pedigrees that have not been simulated with the \code{SimRVpedigree} package must create a ped object using \code{new.ped}.  This ped object \emph{must} contain the following variables for each pedigree member:
#'
#' \tabular{lll}{
...
#' Obtain specialized pedigree information
#'
#' Users who wish to use \code{affInfo.ped} for pedigrees not simulated with the \code{SImRVPedigree} package must ensure that the pedigree, \code{ped_file}, supplied to \code{affInfo.ped} contains the following variables for each pedigree member:
#' \enumerate{
#' \item \code{ID}: an identification number.
#' \item \code{dadID}: identification number of father.
#' \item \code{momID}: identification number of mother.
...
#'
#' @param ped_file Data.frame. A pedigree generated by \code{sim_ped} or \code{sim_RVped}.
#'
#' @return  A list containing the following:
#' @return \item{\code{affVars}}{A data.frame containing information for affected relatives only.}
#' @return \item{\code{affKin}}{A list containing matrices of the pariwise kinship  coefficients for the affected relatives for each family in \code{ped_file.}}
#'
#' @export
#' @importFrom kinship2 kinship
...
#'
#' #information for the affected relatives
#' Ainfo[[1]]
#'
#' # matrix of pairwise kinship coefficents
#' # for the affected relatives in family 4
#' Ainfo[[2]][[4]]
#'
affInfo.ped <- function(ped_file){
  ...
  ped_return = list(affVars = do.call(rbind, afdat),
                    affKin = kindat)
  return(ped_return)
}
...
sumVars <- function(ped_file){
  ...
  TR <- nrow(ped_file)
  NAF <- nrow(AV)
  AOO <- mean(AV$onsetYr - AV$birthYr, na.rm = T)
