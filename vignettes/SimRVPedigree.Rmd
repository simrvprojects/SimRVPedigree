...
The distinguishing feature the `SimRVPedigree` package is that it aims to accurately mimic the process of family development, while allowing users to incorporate multiple facets of a particular acsertainment scheme.  To illustrate and fully explain these features, in the next two sections, we will first discuss several assumptions governing the process of pedigree simulation, followed by a full exposition of the study design features available to users.  Later we provide an example, which illustrates an application of `SimRVPedigree` to simulate pedigrees for a specific family-based study.
...
The `sim_RVpedigree` function is used to randomly simulate families likely to be ascertained under a particular study design or ascertainment scheme.  The study design features available to the user are specified via the following arguements:
...
Based on the description of the family-based study described above, and referring to the input requirements in the last section, to randomly simulate famlies likely to be ascertained by this study with the `sim_RVpedigree` function, users will set:
...
That is, users studying lymphiod cancer, will need to provide the age-specific lymphoid cancer onset hazard, and age-specific death hazard for individuals who have experienced onset of a lympiod cancer (affected population).  These may be estimated by a program such as the Surveillance, Epidemiology, and End Results Program (SEER), however, we are not at liberty to reproduce data obtained through SEER.  Additionally, the uset must provide the age-specific death hazard for individuals who have not experienced onset of a lympiod cancer (unaffected population).  Since death by lymphiod cancer only accounts for a very small percentage of all deaths, the user may choose to approximate death hazard for the unaffected population by the population age-specific death hazard.  

For the sake of illustration, we will pretend that the required age-specific hazards are represented in the `AgeSpecific_Hazards` dataset included in with the `SimRVPedigree` package.  We note that the age-specific hazards in the `AgeSpecific_Hazards` dataset were created solely for the purpose of illustrating package function usage, and do not apply to any disease in particular.  For more information on the `AgeSpecifc_Hazards` dataset type the command `help(AgeSpecific_Hazards)` into the `R` console.
