...
Looking at the output above, we can see that when we call `sim_RVpedigree()`, we will set the `onset_hazard` argument to the first column in the `AgeSpecific_Hazards` dataset, and the `death_hazard` argument will be set to the second and third columns (in that order) of the `AgeSpecific_Hazards` dataset.  Since the `AgeSpecific_Hazards` dataset supplies the age specific hazard rates in yearly increments starting at age 0 and ending with age 100, we will define our partion to feed to the `part` argument of `sim_RVpedigree` as follows:
...
plot(x = seq(0, 99, by = 1), y = AgeSpecific_Hazards[,1],
     xlab = "Age (in years)", ylab = "Baseline Hazard Rate \n of Disease", 
...
Since we are interested in families that were ascertained for study, we will focus on the `ascertain_ped` data frame returned by the `sim_RVpedigree()` function.  Let's look at the information that is contained in this data frame.

```{r}
# store the second data frame, i.e. the ascertained pedigree, 
# returned by the sim_RVpedigree() function
studyPed <- ex_RVped[[2]] 
