...
2. We allow users to choose from one of the following three assumptions:
    + a. `'first'`: Assume that the variant is rare enough that a single copy has been introduced by one founder, and begin the simulation of the pedigree with this founder. (source: Bureau)
    + b. `'multiple'`: Simulate founder genotype status at the disease locus based on the allele frequency of the rare varaint in the population.  NOTE: Under this assumption: founders may be homozygous for the rare variant, multiple founders may introduce the rare variant, and offspring who are homozygous for the rare variant may be observed. (source: Li Ch 1)
...
1. `hazard_rates`: a data frame with 3 columns:
   1. `hazard_rates[, 1]`: the population age-specific hazard rate for lymphoid cancer.
   2. `hazard_rates[, 2]`: the age-specific hazard rate for death in the unaffected population, i.e. individuals who have NOT had lymphoid cancer.
   3. `hazard_rates[, 3]`: the age-specific hazard rate for death in the affected population, i.e. individuals who have had lymphoid cancer.
2. `part`: a partition of ages over which to apply the age-specific hazard rates in `hazard_rates`.
...
RV_peds = foreach(i = seq(npeds),   
                  ...
                  .options.RNG = 1984
                    ...
                    sim_RVped(hazard_rates = LC_hazard,
                              ...
                              GRR = 5, carrier_prob = 1, stop_year = 2015,
                              recall_probs = c(1),
