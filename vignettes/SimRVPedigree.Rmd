...
2. With respect to how the causal variant is introduced to the pedigree we allow users to choose from one of the following three assumptions:
    + a. `'first'`: Assume that the variant is rare enough that a single copy has been introduced by one founder, and begin the simulation of the pedigree with this founder. This setting is due to the assumptions made by Bureau [11].  We note that for diseases with low penetrance this setting may not yeild accurate results. 
    + b. `'multiple'`: Simulate founder genotypes at the disease locus based on the allele frequency of the rare variant in the population under the assumption of random mating (or under Hardy-Weinburg equilibrium).  NOTE: Under this assumption: founders may be homozygous for the rare variant, multiple founders may introduce the rare variant, and offspring who are homozygous for the rare variant may be observed. (source: Li Ch 1 or Lange Ch 1)
    + c. `'single'`: Assume that a single founder introduces a single copy of the variant to the pedigree.  Here we allow successive founders the opportunity to introduce a single copy of the rare variant.  The probability that a founder introduces a single copy of the rare variant to the pedigree is the carrier frequency of the rare variant in the population in the population.  After the rare variant is introduced no other founders are permitted to re-introduce the variant.  NOTE: The founder who introduces the rare variant may not be the same founder we begin the simulation of the pedigree with.  (source: ????)
    
3. We transmit the rare variant from parent to offspring according to Mendel's laws.
...
9. `RVfounder`: The method by which the rare variant is introduced to the pedigree.  We allow three modes of introduction discussed in [Individual and Pedigree Specific Assumptions](#PedLevel): `'first'`, which corresponds to assumption 2.a.; `'multiple'`, which corresponds to assumption 2.b.; and `'single'`, which corresponds to assumption 2.c.
...
12. `NB_params`: the size and probability parameters of the negative binomial distribution used to model the number of children per household. If missing, it is assumed that `NB_params = c(2, 4/7)`, this setting is due to the findings of Kojima and Kelleher [2].
...
To obtain specialized information about the simulated pedigree we supply the ascertained pedigree to the `pedigree_info` function.

```{r}
# obtain information about the ascertained pedigree by suppling it to pedigree_info 
studyPed <- pedigree_info(ascPed)

# View a summary of studyPed
summary(studyPed)
```

The `pedigree_info` function returns a list containing five items. The item first is named `link_format.`  This is simply the pedigree in linkage format; i.e. a data frame which contains only the variables `FamID`, `ID`, `dadID`, `momID`, `sex`, and `affected`. 
...
The second item returned by pedigree_info is named `affected_info` and provides information for the affected relatives.
...
The third item returned by pedigree_info is named `kinshipMatrix` is the kinship matrix for the pedigree obtained by the `kinship` function of the `kinship2` package  (execute the command `help(kinship)` for more information).  The fourth and fifth items returned by `pedigree_info` are useful when plotting pedigrees using the `kinship2` package, which we illustrate in the following code block. 
...
```{r, fig.height = 5.5, fig.width = 7.5}
 # Plot pedigree by passing the fourth item returned by pedigree_info, kinshipPedigree, to the plot() function.
...
Often, there is additional individual information we would like to display along with the pedigree.  For example: an individual's age of onset, and their current age or their age at death.  The following code illustrates how we can use the fifth item returned by `pedigree_info` to accomplish this.

```{r, fig.height = 7, fig.width = 8.5}
# obtain specialized labels for pedigree by supplying label_year = 2017 to pedigree_info
studyPed2017 <- pedigree_info(ascPed, label_year = 2017)

# Plot pedigree with new id labels that reflect the ages of pedigree memebers in 2017.
...
mtext("Calendar Year 2017", side = 3, line = 1)
```
...
# Next, supply Ped2001 and the year of interest to the pedigree_info function.
studyPed2001 <- pedigree_info(Ped2001, label_year = 2001)

# Plot pedigree with new id labels that reflect the ages of pedigree memebers in 2001.
...
pedigree.legend(studyPed2001$kinshipPedigree, 
                location = "topleft", radius = 0.25)
mtext("Calendar Year 2001", side = 3, line = 1)
```

Looking at the pedigree above, we see that in 2001, only 3 family members had experienced onset of lymphoid cancer, two of whom were alive.  Referring to the 2017 pedigree, we see that both disease-affected individuals who were alive in 2001 died by 2017.  Additionally, we see that thirteen years after the pedigree was ascertained the proband's brother (the pedigree member with ID: 3) experienced onset of lymphoid cancer.
...
[11] Alexandre Bureau, Samuel G. Younkin, Margaret M. Parker, Joan E. Bailey-Wilson,
