...
2. With respect to how the causal variant is introduced to the pedigree we allow users to choose from one of the following three assumptions:
    + a. `'first'`: Assume that the variant is rare enough that a single copy has been introduced by one founder, and begin the simulation of the pedigree with this founder, as in [1].
    + b. `'single'`: Simulate the starting founder's rare-variant status with probability equal to the carrier probability of the rare variant in the population.  We note that under this setting pedigrees may not transmit the rare variant.
    
3. When the rare variant is present, we transmit it from parent to offspring according to Mendel's laws.

4. We assume that, given an individual's current age, their time to disease onset is the waiting time in a non-homogeneous Poisson process with an age-specific hazard rate that follows a proportional hazards model.  In this model, individuals who have NOT inherited the rare variant experience disease onset according to the baseline hazard rate of disease.  On the other hand, individuals who have inherited the rare variant are assumed to have an increased risk of disease relative to those who have not inherited it.  The user is expected to supply the population hazard rate of disease, the allele frequency of causal variants, and the relative-risk of disease for genetic cases.
...
4. `allele_freq`: the population allele frequency of the rare variant. 
...
9. `RVfounder`: The method by which the rare variant is introduced to the pedigree.  We allow three modes of introduction discussed in [Individual and Pedigree Specific Assumptions](#PedLevel): `'first'`, which corresponds to assumption 2.a.; `'single'`, which corresponds to assumption 2.b.  By default, `RVfounder = 'first'`.
...
11. `birth_range`: the minimum and maximum allowable ages, in years, between which individuals may reproduce.  If missing, it is assumed that `birth_range = c(18, 45)`.
12. `NB_params`: the size and probability parameters of the negative binomial distribution used to model the number of children per household. If missing, it is assumed that `NB_params = c(2, 4/7)`, this setting is due to the findings of Kojima and Kelleher [3].
13. `stop_year`: the last year of study.  If missing, the current year is used.
...
Suppose that, from 1995 to 2010, researchers ascertained 150 families that contained at least two, related individuals affected by a lymphoid cancer.  Data was collected for the 150 families until the year 2017.  Researchers believe that affected members within a family share a rare variant that increases disease susceptibility.  While they are not willing to assume that the same rare variant is segregating in all families, they are willing to make the simplifying assumption that all individuals carrying a genetic susceptibility variant share the same relative-risk of disease.  They believe that the population allele frequency of causal rare variants is roughly 0.01.  They also believe that carriers of the susceptibility variant are 10 times more likely to develop lymphoid cancer than non-carriers.
...
LC_hazard <- new.hazard(partition = age_part,
...
Finally, we show how the `sim_RVped` function may be used to simulate a single family ascertained for study in the example described at the beginning of this section.  Here we make the assumption that a causal variant is introduced by one of the two eldest founders. 
...
The `sim_RVped` function returns a list containing two data frames.  The first data frame, named `full_ped`, represents the original pedigree, prior to proband selection and trimming.  The second data frame, named `ascertained_ped`, represents the ascertained pedigree, and includes an additional variable to identify the proband.  Any relatives not recalled by the proband are removed from the ascertained pedigree.  If a relative is not recalled by the proband, but is required to plot the pedigree using the `kinship2` package [7], they are included in the ascertained pedigree, but their information is missing, and they are be labelled as unavailable.  

Since we are interested in families that were ascertained for study, we will focus on the `ascertained_ped` data frame returned by the `sim_RVped` function.  We can view the first few rows of `ascertained_ped` by executing the following command. 
...
To obtain specialized information about the simulated pedigree we supply the ascertained pedigree to the `ped_info` function.

```{r}
# obtain information about the ascertained pedigree by suppyling it to ped_info 
studyPed <- ped_info(ascPed)

# View a summary of studyPed
summary(studyPed)
```

The `ped_info` function returns a list containing five items. The item first is named `link_format.`  This is simply the pedigree in linkage format; i.e. a data frame which contains only the variables `FamID`, `ID`, `dadID`, `momID`, `sex`, and `affected`. 

```{r}
# view first three rows of pedigree in linkage format
head(studyPed$link_format, n = 3)
```

The second item returned by ped_info is named `affected_info` and provides information for the affected relatives.

```{r}
# View information for affected relatives
studyPed$affected_info
```

The third item returned by ped_info is named `kinshipMat` is the kinship matrix for the pedigree obtained by the `kinship` function of the `kinship2` package  (execute the command `help(kinship)` for more information).  The fourth and fifth items returned by `ped_info` are useful when plotting pedigrees using the `kinship2` package, which we illustrate in the following code block. 
...
```{r, fig.height = 5.5, fig.width = 7.5}
 # Plot pedigree by passing the fourth item returned by ped_info, kinshipPedigree, to the plot() function.
 plot(studyPed$kinshipPedigree)

 # Plot legend for shaded variables.
 pedigree.legend(studyPed$kinshipPedigree, location = "topleft",  radius = 0.25)
```

Often, there is additional individual information we would like to display along with the pedigree; for example: an individual's age of onset, and their current age or their age at death at a given reference year.  The following code illustrates how we can use the fifth item returned by `ped_info` to accomplish this for the year 2017.

```{r, fig.height = 7, fig.width = 8.5}
# obtain specialized labels for pedigree by supplying ref_year = 2017 to ped_info
studyPed2017 <- ped_info(ascPed, ref_year = 2017)

# Plot pedigree with labels that reflect the ages of pedigree memebers in 2017.
plot(studyPed2017$kinshipPedigree, cex = 0.9, 
     id = studyPed2017$pedLabs,
     mar = c(1.1, 3.1, 5.1, 2.1))

# Plot pedigree legend.
pedigree.legend(studyPed2017$kinshipPedigree, 
                location = "topleft", radius = 0.25)
mtext("Reference Year: 2017", side = 3, line = 1)
```

It is important to note that the pedigree above may have looked quite different at the time of ascertainment.  For pedigrees simulated by `sim_RVped`, we assume that ascertainment occurs during the same calendar year that the proband experiences disease onset. We can determine what year the pedigree above was ascertained using the following command.
...
From the output above we see that this pedigree was ascertained in 2001.  To view the pedigree as it would have appeared at the time of ascertainment, we can use the `censor_ped` function to censor all events that occurred after 2001.  Next, we supply the censored pedigree to the `ped_info` function and use the output to plot the pedigree with updated labels.  The following code illustrates this process.

```{r, fig.height = 6.5, fig.width = 8.5}
# First, we censor all events that occurred after 2001. 
Ped2001 <- censor_ped(ped_file = ascPed, censor_year = 2001)

# Next, supply Ped2001 and the year of interest to the ped_info function.
studyPed2001 <- ped_info(Ped2001, ref_year = 2001)

# Plot pedigree with new labels that reflect the ages of pedigree members in 2001.
plot(studyPed2001$kinshipPedigree, cex = 0.9, 
     id = studyPed2001$pedLabs,
     mar = c(1.1, 3.1, 5.1, 2.1))

# Plot pedigree legend.
pedigree.legend(studyPed2001$kinshipPedigree, 
                location = "topleft", radius = 0.25)
mtext("Reference Year: 2001", side = 3, line = 1)
```

Looking at the pedigree above, we see that in 2001, only 3 family members had experienced onset of lymphoid cancer, two of whom were alive (the pedigree members with IDs 6 and 7).  Referring to the 2017 pedigree, we see that both disease-affected individuals who were alive in 2001 died by 2017.  Additionally, we see that at age 77, thirteen years after the pedigree was ascertained, the proband's brother (the pedigree member with ID 3) experienced onset of lymphoid cancer.
