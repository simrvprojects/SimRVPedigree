...
10. `birth_range`: the minimum and maximum allowable ages between which individuals may reproduce.  If missing, it is assumed that `birth_range = c(18, 45)`.
11. `NB_params`: the size and probability parameters of the negative binomial distribution used to model the number of children per household. If missing, it is assumed that `NB_params = c(2, 4/7)`, [cite Kojima and Kelleher]
...
For individuals studying lymphoid cancer in the United States, the age-specific hazard rates of lymphoid cancer onset and death in the affected population may be estimated by a program such as the Surveillance, Epidemiology, and End Results Program (SEER) [3].  Furthermore, since death by lymphoid cancer accounts for a relatively small proportion of all death causes, the user may choose to approximate the hazard rates of death in the unaffected population by the age-specific hazard rates of death in the general population, which can be estimated from actuarial life tables provided by the Social Security Administration [4].

For the sake of illustration, suppose that the family study in the example was conducted in the United States.  The `AgeSpecific_Hazards` data set included with the `SimRVPedigree` package contains age-specific hazard rates which were generated to roughly mimic those required for this particular example.  Each of the three columns in the `AgeSpecific_Hazards` data set provide the required age-specific hazard rates, in yearly increments, beginning at age 0 and ending with age 100.  That is, the values in the first row describe the hazard rates for an individual whose age is contained in the interval [0, 1), while the values in the second row describe the hazard rates for an individual whose age is contained in the interval [1, 2), and so on.  For more information on the `AgeSpecifc_Hazards` data set type the command `help(AgeSpecific_Hazards)` into the `R` console.
...
#investigate columns to see where particular hazard rates are located  
colnames(AgeSpecific_Hazards)
```

First, we define the hazard rates and partition of ages required by `sim_RVpedigree()`. 
...
Let's quickly plot the age-specific hazard rates against the ages that they apply to.  
...
legend("topleft", title = "Population", 
       legend = c("General", "Affected"),
       col = c("dodgerblue", "goldenrod2"), lwd = 2)
#reset plot defaults
par(mfrow = c(1, 1))
```

Finally, we illustrate the use `sim_RVpedigree()` to simulate a single family ascertained for study in the example described at the beginning of this section.
```{r}
...
The `sim_RVpedigree` returns a list of two data frames.  The first data frame, named `full_ped`, represents the original pedigree prior to proband selection and trimming.  The second data frame, named `ascertain_ped`, represents the ascertained pedigree, and includes an additional variable to identify the proband.  Any relatives not recalled by the proband are removed from the ascertained pedigree.  If a relative is not recalled by the proband, but is required to plot the pedigree using the `kinship2` package [5], they are included in the ascertained pedigree, but their information is missing, and they are be labelled as unavailable.  

Since we are interested in families that were ascertained for study, we will focus on the second item returned by `sim_RVpedigree`.  Let's quickly take a look at the information that is contained in the ascertained pedigree.
```{r}
#Print first three rows of the second dataframe, 
# i.e. the ascertained pedigree, returned by sim_RVpedigree
head(ex_RVped[[2]], n = 3)
```
The columns in the data frame above may be described as follows:
 ...
 7. `DA1`: first allele at disease locus: `DA1 = 1` if rare variant, and 0 otherwise.
 8. `DA2`: second allele at disease locus: `DA2 = 1` if rare variant, and 0 otherwise.
 ...
 14. `Gen`: the generation in the pedigree to which the individual belongs. 
...
#Define pedigree by passing all the required information about the ascertained 
#pedigree, ex_RVped[[2]], to kinship2's pedigree() function.
 ex_RVpedigree <- pedigree(id = ex_RVped[[2]]$ID,
                           ...
                           affected = cbind(Affected = ex_RVped[[2]]$affected,
                                            ...
                                            RV_status = ex_RVped[[2]]$DA1 + 
                                                        ex_RVped[[2]]$DA2),
                           status = ifelse(is.na(ex_RVped[[2]]$death_year), 0, 1))
...
Often, there is additional individual information we would like to display along with the pedigree.  For example: an individual's age of onset, and their current age or, if they have already died, their age at death.  The following code illustrates how we could annotate the pedigree above to include this information.
```{r, fig.height = 7, fig.width = 7}
#create a current age label for individuals for whom we have a year of birth
CurrentAge <- ifelse(is.na(ex_RVped[[2]]$birth_year), 
                     " ", paste0("\n age: ", 2015 - ex_RVped[[2]]$birth_year))

#create a death age label for individuals who have died by the current year
DeathAge <- ifelse(is.na(ex_RVped[[2]]$death_year), 
                   " ", paste0("\n death age: ", 
                               ex_RVped[[2]]$death_year - 
                                 ex_RVped[[2]]$birth_year))

#create an onset age label for individuals who have 
# experienced onset by the current year
OnsetAge <- ifelse(is.na(ex_RVped[[2]]$onset_year), 
                   " ", paste0("\n onset age: ", 
                               ex_RVped[[2]]$onset_year - 
                                 ex_RVped[[2]]$birth_year))

#plot pedigree with new id labels
plot(ex_RVpedigree, cex = 0.9, 
     id = paste0("ID: ", sep = "", ex_RVped[[2]]$ID,
                 CurrentAge, OnsetAge, DeathAge),
     mar = c(1.1, 3.1, 5.1, 2.1))
pedigree.legend(ex_RVpedigree, location = "topleft", radius = 0.25)
mtext("Calendar Year 2015", side = 3, line = 1)
```

It is important to note that the pedigree above may have looked quite different at the time of ascertainment.  Recall that, for pedigrees simulated with `sim_RVpedigree()`, ascertainment of the pedigree is assumed to occur during the same calender year that the proband experienced disease onset.  We can determine what year the pedigree above was ascertained using the following commands.

```{r}
Ascertain_Year <- ex_RVped[[2]]$onset_year[which(ex_RVped[[2]]$proband == 1)]
Ascertain_Year
```
...
Ped2005 <- censor_ped(ped_file = ex_RVped[[2]], censor_year = 2005)

#Next, create lables for the censored pedigree
#create a 2005 age label for individuals for whom we have a year of birth
Age2005 <- ifelse(is.na(Ped2005$birth_year), 
                     " ", paste0("\n age: ", 2005 - Ped2005$birth_year))

#create a death age label for individuals who have died by 2005
DeathAge2005 <- ifelse(is.na(Ped2005$death_year), 
                   " ", paste0("\n death age: ", 
                               Ped2005$death_year - 
                                 Ped2005$birth_year))

#create an onset age label for individuals who experienced onset by 2005
OnsetAge2005 <- ifelse(is.na(Ped2005$onset_year), 
                   " ", paste0("\n onset age: ", 
                               Ped2005$onset_year - 
                                 Ped2005$birth_year))

# Define pedigree object for 2005 pedigree.  Since there are different 
# ways to define a pedigree object, for variety, this time we'll do it 
# a bit differently and use the with() function to pass the censored
# pedigree to the pedigree() function.  
Ascertained_Ped <- with(Ped2005, 
                        pedigree(id = ID,
                                 dadid = dad_id,
                                 momid = mom_id,
                                 sex = (gender + 1),
...
#plot pedigree with new id labels
plot(Ascertained_Ped, cex = 0.9, 
     id = paste0("ID: ", sep = "", Ped2005$ID,
                 Age2005, OnsetAge2005, DeathAge2005),
     mar = c(1.1, 3.1, 5.1, 2.1))
pedigree.legend(Ascertained_Ped, location = "topleft", radius = 0.25)
mtext("Calendar Year 2005", side = 3, line = 1)
```

Looking at the pedigree above, we see that in 2005, only 2 family members had experienced onset of lymphoid cancer, both of whom were alive.  Referring back to the previous pedigree, we see that both individuals who were affected in 2005 have died by 2015.  Additionally, we see that nine years after the pedigree was ascertained the proband's sister (the pedigree member with ID 5) also experienced onset of lymphiod cancer.
...
For details concerning any of the arguments of the `sim_RVpedigree()` function please refer to section 3.

To illustrate the effect of the relative risk argument on the time required to simulate a study sample of 150 families for the family-study described in section 5, the following table lists the average computation times, in hours, observed over repeated simulations.
...
[3] The Surveillance, Epidemiology, and End Results (SEER) Program.  Retrieved on 24 October 2016 from \url{http://seer.cancer.gov/}

[4] Social Security Administration. Life Tables for the United States Social Security Area 1900-2100. Retrieved on 24 October 2016 from \url{https://www.ssa.gov/oact/NOTES/as120/TOC.html}

[5] Terry M Therneau and Jason Sinnwell (2015).
  **kinship2: Pedigree Functions.** 
  *R package version 1.6.4.* 
  https://CRAN.R-project.org/package=kinship2

[6] Ellen M. Wijsman (2012). 
