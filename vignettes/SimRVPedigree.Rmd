...
2. [Basic Usage](#BasicUsage)
    ...
    2. [Simulating Pedigrees](#PedSim)
    3. [Trimming Pedigrees](#PedTrim)
    4. [Simulating Pedigrees with Multiple Affecteds](#RVPedSim)
        1. [Parallel Processing Example](#ParProc)
    5. [Reassigning Generation Based on Affected Status](#AffGen)

## Introduction <a name="introduction"></a>
The `SimRVPedigree` package provides methods and functions to randomly simulate pedigrees that have been ascertained to contain multiple relatives affected with a rare disease.  

Then you could talk about the study design; e.g. ascertainment occurs over a user-specified time-period, and is through the proband. Discuss the rare disease and how we assume that (i) sporadic cases have age-dependent baseline hazard rates which are to be specified by the user, (ii) genetic cases within the same family are due to a shared rare variant but that different families may segregate different rare variants, and 
...
Pedigrees include individual specific variables such as birth year, and, when applicable, onset year, and/or death year.  Additionally, the transmission of a rare variant, assumed to increase disease susceptibility, is simulated according to Mendel's laws.

In addition to randomly simulating pedigrees, `SimRVPedigree` also includes a function to reassign the generation number of based on affected status.
