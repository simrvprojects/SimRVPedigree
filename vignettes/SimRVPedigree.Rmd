...
Researchers are confident that all affected family members have been fully ascertained.  Upon inspecting the founders in the ascertained families, they see that all were born between 1900 and 1980.
...
The `sim_RVped` function returns a list containing two data frames and two numeric constants [EXPLAIN HERE!].  The first data frame, named `full_ped`, represents the original pedigree, prior to proband selection and trimming.  The second data frame, named `ascertain_ped`, represents the ascertained pedigree, and includes an additional variable to identify the proband.  Any relatives not recalled by the proband are removed from the ascertained pedigree.  If a relative is not recalled by the proband, but is required to plot the pedigree using the `kinship2` package [6], they are included in the ascertained pedigree, but their information is missing, and they are be labelled as unavailable.  
...
Age2015 <- ifelse(is.na(studyPed$birthYr) | !is.na(studyPed$deathYr), 
                     " ", paste0("\n age: ", 2015 - studyPed$birthYr))

# Create a death age label for individuals who have died by 2015.
DeathAge2015 <- ifelse(is.na(studyPed$deathYr), 
                   " ", paste0("\n death age: ", 
                               studyPed$deathYr - studyPed$birthYr))

# Create an onset age label for individuals who experienced onset by 2015.
OnsetAge2015 <- ifelse(is.na(studyPed$onsetYr), 
                   " ", paste0("\n onset age: ", 
                               studyPed$onsetYr - studyPed$birthYr))
...
studyPed$onsetYr[which(studyPed$proband == 1)]
```
...
Age2005 <- ifelse(is.na(Ped2005$birthYr) | !is.na(Ped2005$deathYr),
                  " ", paste0("\n age: ", 2005 - Ped2005$birthYr))

# Next, we create a death age label for individuals who had died by 2005
DeathAge2005 <- ifelse(is.na(Ped2005$deathYr), 
                       " ", paste0("\n death age: ", 
                                   Ped2005$deathYr -  Ped2005$birthYr))

# Finally, we create an onset age label for individuals who had 
# experienced disease onset by the year 2005.
OnsetAge2005 <- ifelse(is.na(Ped2005$onsetYr), 
                       " ", paste0("\n onset age: ",
                                   Ped2005$onsetYr - Ped2005$birthYr))
...
Censored_Ped <- with(Ped2005, 
                     pedigree(id = ID,
                              ...
                              affected = cbind(Affected = affected,
                                               Proband = proband,
                                               RV_status = DA1 + DA2),
                              status = ifelse(is.na(deathYr), 0, 1)))
...
It is important to note that the processing time required to simulate a sample of pedigrees ascertained for multiple affected members is directly related to (1) the rarity of the disease, and (2) arguments specified by the user.  In particular, we expect to see an increase in the required computation time as: 
 ...
 1. the gentic relative-risk, `GRR`, specified by the user approaches 1, particularly for rare diseases,
...
RV_peds = foreach(i = seq(npeds),   
                  ...
                  .options.RNG = 1984
                    ...
                    sim_RVped(hazard_rates = LC_hazard,
                              ...
                              GRR = 5, prob_causalRV = 1, stop_year = 2015,
                              recall_probs = c(1),
