...
2. We allow users to choose from one of the following three assumptions:
    + a. Assume that the variant is rare enough that a single copy has been introduced by one founder, and begin the simulation of the pedigree with this founder. (Bureau)
    + b. Assume that a single founder introduces a single copy of the variant to the pedigree.  Here we allow successive founders the opportunity to introduce the rare variant with probability proprotional to its allele frequency in the population until the variant is introduced to the pedigree.  After the rare variant is introduced no other founders are permitted to re-introduce the varaint. NOTE: The founder who introduces the rare variant may not be the same founder we begin the simulation of the pedigree with.
    + c. Simulate founder genotype status at the disease locus based on the allele frequency of the rare varaint in the population.  NOTE: Under this assumption: founders may be homozygous for the rare variant, multiple founders may introduce the rare variant, and offspring who are homozygous for the rare variant may be observed. (Li Ch 1)
...
1. `hazard_rates`: a data frame with:
...
4. `FamID`: the family ID to assign to the simulated pedigree.
5. `founder_byears`: the span of years from which to simulate, uniformly, the founder's birth year.
6. `ascertain_span`: the year span of the ascertainment period.  This period represents the range of years during which the proband developed disease and the family would have been ascertained for multiple affected relatives. 
7. `num_affected`: the minimum number of affected individuals in the pedigree. 

Optional arguments:

8. `recall_probs`: the proband's recall probabilities for relatives, as described in section 3.  If missing, four times kinship coefficient between the proband and the relative is used.
9. `birth_range`: the minimum and maximum allowable ages, in years, between which individuals may reproduce.  If missing, it is assumed that `birth_range = c(18, 45)`.
10. `NB_params`: the size and probability parameters of the negative binomial distribution used to model the number of children per household. If missing, it is assumed that `NB_params = c(2, 4/7)`, this setting is due to the findings of Kojima and Kelleher [2].
11. `stop_year`: the last year of study.  If missing, the current year is used.
...
Finally, we show how `sim_RVped()` may be used to simulate a single family ascertained for study in the example described at the beginning of this section.  Here we make the assumption that a causal variant is introduced by one of the two eldest founders. 
...
From the output above we see that the example pedigree was ascertained in 2001.  To view the pedigree as it would have appeared at the time of ascertainment, we can use the `censor_ped()` function to censor all events that occurred after 2001.  The following code illustrates this process.
...
# Next, supply Ped2001 and the associated label year to the pedigree info function.
...
Looking at the pedigree above, we see that in 2001, only 3 family members had experienced onset of lymphoid cancer, one of whom was alive.  Referring to the 2015 pedigree, we see that both individuals who were affected in 2005 have died by 2015.  Additionally, we see that one year after the pedigree was ascertained the proband's father (the pedigree member with ID: 3) also experienced onset of lymphoid cancer.
