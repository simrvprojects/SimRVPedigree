...
plot(ascPed, legendLocation = "bottomleft")
```

The legend, in the upper left corner of the plot, provides information for the shading seen in certain pedigree members.   Looking at the legend, we see that individuals who are affected by disease (IDs 1, 3, 4, and 5) are indicated by a solid shading in the upper left third of their symbol.  The proband (ID 5) is indicated by shading in the lower third of an individual's symbol.  Individuals who have inherited a causal variant (IDs 1, 4, and 10) are indicated by shading in the upper right third of their symbol.
...
# Plot pedigree with FamID = 1
plot(Bpeds[Bpeds$FamID == 1,])

# Re-assign the generation numbers among the affected members
# in the pedigree with FamID = 1 and store it as Aped1
Aped1 <- reassignGen.ped(Bpeds[Bpeds$FamID == 1, ])

# Print before and after generation numbers for the disease-affected relatives
cbind(ID = Bpeds[Bpeds$FamID == 1 & Bpeds$affected, 2],
      Gen_Before = Bpeds[Bpeds$FamID == 1 & Bpeds$affected, 14],
      Gen_After = Aped1[Aped1$affected, 14])
```

Looking at the output above we see that the generation numbers of the affected members have not changed.  Referring back to the plotted pedigree we see that this is because the most recent common ancestors of the affected individuals (IDs 6, 8, and 15) are the two eldest founders (IDs 1 and 2).

We note that, `Aped1` is a reduced version of the original pedigree and will contain only affected members, their parents, and common ancestors.  Since the focus of this assignment scheme is on the affected members, all other family members will now have missing generation numbers. 

```{r, fig.height = 5.5, fig.width = 6.5}
#plot pedigree with reassigned generation numbers
plot(Aped1,
     legendLocation = "bottomleft")

# Print generation numbers of unaffected relatives after reassignment.
# Notice that after reassignment they are all NA.
Aped1$Gen[Aped1$affected == FALSE]
```


```{r, fig.height = 5.5, fig.width = 8}
# Plot pedigree with FamID = 5
plot(Bpeds[Bpeds$FamID == 5,])

# Re-assign the generation numbers among the affected members
# in the pedigree with FamID = 5 and store it as Aped5
Aped5 <- reassignGen.ped(Bpeds[Bpeds$FamID == 5, ])
...
# Print before and after generation numbers for the disease-affected relatives
cbind(ID = Bpeds[Bpeds$FamID == 5 & Bpeds$affected, 2],
      Gen_Before = Bpeds[Bpeds$FamID == 5 & Bpeds$affected, 14],
      Gen_After = Aped5[Aped5$affected, 14])
```

In this example, referring to the plotted pedigree, we see a pair of affected cousins (IDs 15 and 33).  Therefore, each of the cousins is assigned generation 3, since the most recent common ancestors of the affected cousins are their common grandparents (IDs 3 and 9). 
