...
```{r}
# obtain information about the ascertained pedigree by supplying it to ped_info 
...
From the output above we see that this pedigree was ascertained in 1999.  To view the pedigree as it would have appeared at the time of ascertainment, we can use the `censor_ped` function to censor all events that occurred after 1999.  Next, we supply the censored pedigree to the `ped_info` function and use the output to plot the pedigree with updated labels.  The following code illustrates this process.

```{r, fig.height = 6.5, fig.width = 8.5}
# First, we censor all events that occurred after 1999. 
Ped1999 <- censor_ped(ped_file = ascPed, censor_year = 1999)

# Next, supply Ped1999 and the year of interest to the ped_info function.
studyPed1999 <- ped_info(Ped1999, ref_year = 1999)

# Plot pedigree with new labels that reflect the ages of pedigree members in 1999.
plot(studyPed1999$kinshipPedigree, cex = 0.9, 
     id = studyPed1999$pedLabs,
     mar = c(1.1, 3.1, 5.1, 2.1))

# Plot pedigree legend.
pedigree.legend(studyPed1999$kinshipPedigree, 
                location = "topleft", radius = 0.25)
mtext("Reference Year: 1999", side = 3, line = 1)
```

Looking at the pedigree above, we see that in 1999, only 2 family members had experienced onset of lymphoid cancer, one of whom was alive (the pedigree member with IDs 6).  Referring to the 2017 pedigree, we see that both disease-affected individuals who were alive in 1999 died by 2017.  Additionally, we see that at age 48, twelve years after the pedigree was ascertained, the proband's sister (the pedigree member with ID 7) experienced onset of lymphoid cancer.
