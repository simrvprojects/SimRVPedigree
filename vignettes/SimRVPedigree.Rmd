...
Since we are interested in families that were ascertained for study, we will focus on the `ascertained_ped` data frame returned by the `sim_RVped` function.  We can view a summary or the first three rows of `ascertained_ped` by executing the following commands. 
...
To obtain specialized information about the simulated pedigree we supply the ascertained pedigree to the `affInfo.ped` function.

```{r}
# obtain information about the ascertained pedigree by supplying it to affInfo.ped 
pedInfo <- affInfo.ped(ascPed)

# View a summary of the items returned by affInfo.ped
summary(pedInfo)
```

The `affInfo.ped` function returns a list containing two items. The item first is named `affVars.`  This is a dataframe containing data for the affected relatives. 
...
```{r}
# View matrix of the pairwise kinship coefficients for the affected relatives.
pedInfo$affKin
```

To plot pedigrees we make use of the `kinship2` package; however, to do so we must first use the `ped2pedigree` function, which creates `pedigree` objects from `ped` objects. 
...
# obtain specialized labels for pedigree by supplying the ped object ascPed and ref_year = 2017 to pedigreeLabels
