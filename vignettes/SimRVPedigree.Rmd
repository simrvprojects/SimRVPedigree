...
\newpage
Now we show how `simRVped` may be used to simulate a pedigree ascertained for the previously described study.  Here we make the assumption that a causal variant is introduced by one of the two eldest founders. 
...
plot(ascPed, ref_year = 2017,
     ...
     mar = c(1, 2, 3, 2))
```
When `ref_year` is supplied, additional information will be plotted when possible.  If year of birth and year of death information is provided, the pedigree will contain an age identifier for individuals who alive at the specified reference year.  If the individual has died, their year of birth and death is displayed instead.  If year of disease-onset data is provided, the disease-onset age will be displayed below individuals who have experienced disease-onset by the selected reference year.  For example, the pedigree member with ID 1 was born in 1914,  died in 1959, and experienced disease onset at age 38.
...
# create a ped object 
Bped1 <- new.ped(EgPeds[EgPeds$FamID == 1, ])
...
Aped1 <- reassignGen.ped(Bped1)

# Print before and after generations for the disease-affected relatives
cbind(ID = Bped1$ID[Bped1$affected],
      Gen_Before = Bped1$Gen[Bped1$affected],
      Gen_After  = Aped1$Gen[Aped1$affected])
```
...
Bped5 <- new.ped(EgPeds[EgPeds$FamID == 5, ])

# Plot pedigree
plot(Bped5)

# Re-assign the generation numbers and store as Aped5
Aped5 <- reassignGen.ped(Bped5)

# Print before and after generations for the disease-affected relatives
cbind(ID = Bped5$ID[Bped5$affected],
      Gen_Before = Bped5$Gen[Bped5$affected],
      Gen_After  = Aped5$Gen[Aped5$affected])
```
