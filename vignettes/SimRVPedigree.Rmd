...
2. With respect to how the causal variant is introduced to the pedigree we allow users to choose from one of the following two assumptions:
    ...
    + b. `'single'`: Simulate the starting founder's rare-variant status with probability equal to the carrier probability of the rare variant in the population.  We note that under this setting pedigrees may not segregate the rare variant.
...
11. `segregate_RV`: A logical variable indicating if we shoudl assume that all pedigrees segregate the rare variant.  When `segregate_RV = TRUE` all pedigrees will segregate the causal rare-variant.  However, when `segregate_RV = FALSE` we allow the starting founder to introduce the causal variant with probability equal to `carrier_prob`.  Therefore, when `segregate_RV = FALSE` a simulated pedigree may be comprised entirely of sporadic cases.  By default, `segregate_RV = FALSE`.
...
The third item returned by ped_info is named `kinshipMat` and is the kinship matrix for the pedigree obtained by the `kinship` function of the `kinship2` package  (execute the command `help(kinship)` for more information).  The fourth and fifth items returned by `ped_info` are useful when plotting pedigrees using the `kinship2` package, which we illustrate in the following code block. 
...
From the output above we see that this pedigree was ascertained in 2003.  To view the pedigree as it would have appeared at the time of ascertainment, we can use the `censor_ped` function to censor all events that occurred after 2003.  Next, we supply the censored pedigree to the `ped_info` function and use the output to plot the pedigree with updated labels.  The following code illustrates this process.

```{r, fig.height = 6.5, fig.width = 8.5}
# First, we censor all events that occurred after 2003. 
Ped2003 <- censor_ped(ped_file = ascPed, censor_year = 2003)

# Next, supply Ped2003 and the year of interest to the ped_info function.
studyPed2003 <- ped_info(Ped2003, ref_year = 2003)

# Plot pedigree with new labels that reflect the ages of pedigree members in 2003.
plot(studyPed2003$kinshipPedigree, cex = 0.9, 
     id = studyPed2003$pedLabs,
     mar = c(1.1, 3.1, 5.1, 2.1))

# Plot pedigree legend.
pedigree.legend(studyPed2003$kinshipPedigree, 
                location = "topleft", radius = 0.25)
mtext("Reference Year: 2003", side = 3, line = 1)
```

Looking at the pedigree above, we see that in 2003, only 2 family members had experienced onset of lymphoid cancer, one of whom was alive (the pedigree member with ID 3).  Referring to the 2017 pedigree, we see that the proband died five years after the pedigree was ascertained.  Additionally, we see that at age 53, ten years after the pedigree was ascertained, the proband's son (the pedigree member with ID 7) experienced onset of lymphoid cancer.
